...
function showOtpStep(isDemo = false) {
  authPhone = $("#phoneInput").value.replace(/\D/g, "");
  $("#phonePreview").textContent = `+91 ${authPhone.slice(0, 2)}••••••${authPhone.slice(-2)}`;
  $("#authForm").hidden = true;
  $("#otpForm").hidden = false;
  $("#demoOtp").hidden = !isDemo;
  $("#otpInput").focus();
  clearAuthError();
}

function finishLogin() {
  localStorage.setItem("schemesaathi-authenticated", "true");
  $("#authGate").classList.add("authenticated");
  document.body.classList.remove("auth-locked");
  clearAuthError();
  if (localStorage.getItem("schemesaathi-profile-v2-complete") !== "true") {
    setTimeout(() => openProfile(), 0);
  }
}

if (localStorage.getItem("schemesaathi-authenticated") === "true") finishLogin();
$("#authForm").addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch(`${API_BASE}/auth/request-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone }) });
    if (!response.ok) {
      const payload = await response.json().catch(() => ({}));
      throw new Error(payload.detail || "OTP request failed");
    }
    const payload = await response.json();
    showOtpStep(payload.demo === true);
    return;
  } catch (error) {
    showAuthError(error.message || "We could not send an OTP. Please try again.");
    return;
  }
});
$("#otpForm").addEventListener("submit", async (event) => {
  ...
  let verified = false;
  try {
    const response = await fetch(`${API_BASE}/auth/verify-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone: authPhone, otp }) });
    if (response.ok) {
      const payload = await response.json();
      localStorage.setItem("schemesaathi-token", payload.access_token);
      verified = true;
    }
  } catch (error) {
    showAuthError("We could not verify the OTP. Please check your connection and try again.");
    return;
  }
  if (!verified) {
    showAuthError("That OTP is not correct. In demo mode, use 123456.");
    return;
  }
  finishLogin();
});
...
$("#resendOtp").addEventListener("click", () => {
  showAuthError("A fresh demo OTP was sent. Use 123456 to continue.");
